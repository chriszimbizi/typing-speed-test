...
// Load a random paragraph from the array and display it
const loadParagraph = () => {
  const randomIndex = Math.floor(Math.random() * paragraphs.length);
  const paragraph = paragraphs[randomIndex];

  // Convert each character into a <span> element and join them as a string
  const spans = [...paragraph].map((char) => `<span>${char}</span>`).join("");
...
const initTyping = () => {
  // Get all the <span> elements representing characters in the paragraph
  let chars = typingText.querySelectorAll("span");

  // Get the character that the user has typed for the current position (charIndex)
  ...
  if (charIndex > chars.length - 1 || timeLeft <= 0) {
    ...
    chars.forEach((span) => span.classList.remove("active")); // Remove the "active" class from all characters
    chars[charIndex].classList.add("active"); // Add the "active" class to the current character

    // Calculate and update WPM, accuracy, and CPM
    ...
    wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm; // Handle edge cases for WPM

    accuracyTag.textContent = `${accuracy}%`; // Update accuracy in the UI
    wpmTag.textContent = wpm; // Update WPM in the UI
    cpmTag.textContent = charIndex - mistakes; // Update CPM in the UI
  }
};

// Update the timer and handle its completion
const initTimer = () => {
  if (timeLeft > 0) {
    timeLeft--;
    timeTag.textContent = timeLeft; // Update the time remaining in the UI
    let wpm = Math.round(
      ((charIndex - mistakes) / 5 / (maxTime - timeLeft)) * 60
    );
...
const resetGame = () => {
  ...
  accuracyTag.textContent = 100;
  cpmTag.textContent = 0;
};

// Initial setup
loadParagraph(); // Load a random paragraph
inputField.addEventListener("input", initTyping); // Add event listener for input
